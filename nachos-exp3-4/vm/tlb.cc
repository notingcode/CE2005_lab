...
void InsertToTLB(int vpn, int phyPage)
{
  ...
  for(int j = 0; j < TLBSize; j++){
    printf("TLB: vpn=%d / phy=%d / valid=%d%c\n", machine->tlb[j].virtualPage, machine->tlb[j].physicalPage, machine->tlb[j].valid, j == i ? '*' : ' ');
  }
  printf("\n");
...
int PageOutPageIn(int vpn)
{
  ...
  for(int i=0; i < NumPhysPages; i++){
    printf("IPT: pid = %d / vPage = %d / lastUsed = %d / valid = %d%c\n", memoryTable[i].pid, memoryTable[i].vPage, memoryTable[i].lastUsed, memoryTable[i].valid, phyPage == i ? '*' : ' ');
  }
...
int lruAlgorithm(void)
{
  ...
  if(phyPage < 0) {
    phyPage = 0;
    lastUsed = memoryTable[phyPage].lastUsed; 
